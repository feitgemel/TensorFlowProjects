"""
Spine Fracture Model Inference Script

Description:
This script loads a trained CNN model and uses it to predict whether an X-ray image
shows a spine fracture. It includes image preprocessing, model loading, prediction,
and visualization of results.

Purpose:
- Load and preprocess test images
- Make predictions using the trained model
- Visualize results with the prediction overlay
"""
...
test_img = "D:/Data-Sets-Image-Classification/cervical fracture/test/fracture/CSFDV1B10 (1).png"  # Path to single test image
model_path = "D:/Data-Sets-Image-Classification/cervical fracture/model.keras"                     # Path to saved model
...
image = cv2.imread(test_img)      # Load image in BGR format
image_rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)  # Convert to RGB format

# Image preprocessing steps:
# 1. Resize the image to match model's expected input size
...
print("Raw predictions:", predictions)  # Shows probability distribution across classes
...
plt.title(f"Predicted class: {predicted_class}\nTrue: fracture",
          fontsize=14)           # Show prediction in title
